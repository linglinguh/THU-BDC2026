...
| **输出格式** | `result.csv`（UTF-8编码）：`stock_id,weight` 表头 | ✅ 严格匹配 |
| **输出规范** | ≤5行，每行 `代码,权重`，权重和 ≤1，逗号分隔 | ✅ |
...
- 每阶段结束后平台关闭提交系统，汇总评测，一周后返回成绩及排行榜
- 一天内多次提交，新版本覆盖旧版本

### 0.4 Docker 提交与评审规范（来源：提交&评审.md + 通知公告.md）

| 规范项 | 比赛要求 | 本系统实现 |
|---|---|---|
| **提交方式** | 先提交 result.csv，排名靠前再提交 docker | ✅ |
| **入围比例** | A阶段前20名；B阶段各奖项名额上限的 120% | — |
...
| **算法贡献** | 必须有机器学习算法贡献，否则剔除 | ✅ StockTransformer |
| **复现不一致** | 复现结果与提交结果不一致 → 剔除成绩并取消获奖资格 | — |
| **决赛晋级** | B阶段通过审核的队伍中，前3名在校学生队伍直接晋级；其余按B榜前3名晋级 | — |
